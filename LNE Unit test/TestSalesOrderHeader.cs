...
namespace LNE_Unit_test
{
    public class TestSalesOrderHeader
    {
        ...
        [Fact]
        public void OrderAmount_ShouldReturnSumOfOrderLines()
        {
            // Arrange
            var salesOrder = new SalesOrderHeader();
            var orderLine1 = new SalesOrderLine { Quantity = 2, Price = 10.00 }; // Amount = 20
            var orderLine2 = new SalesOrderLine { Quantity = 1, Price = 15.00 }; // Amount = 15

            salesOrder.AddOrderLine(orderLine1);
            salesOrder.AddOrderLine(orderLine2);

            // Act
            var totalAmount = salesOrder.OrderAmount;

            // Assert
            Assert.Equal(35.00, totalAmount); // 20 + 15 = 35
        }
